...
## answer
In general we want to discourage any use of in inheritance if not used with discriminator hence the warning. I assume you have to do this because there is a non standard resource that can't do `model is ProxyResource`? If so probably have to suppress that too
